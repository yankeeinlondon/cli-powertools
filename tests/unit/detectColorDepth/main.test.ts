...
describe("detectColorDepth()", () => {
    ...
    it("type tests", async () => {
        ...
        // Runtime check that result is one of the ColorDepth values
        expect([8, 16, 256, 16700000]).toContain(result);

        type cases = [
            // Return type should be Promise<ColorDepth>
            Expect<ReturnType<typeof detectColorDepth> extends Promise<ColorDepth> ? true : false>,
            // Awaited return should be ColorDepth union
            Expect<Awaited<ReturnType<typeof detectColorDepth>> extends ColorDepth ? true : false>,
            // ColorDepth should allow all four values
            Expect<ColorDepth extends 8 | 16 | 256 | 16700000 ? true : false>,
            // Function should accept no parameters
            Expect<Parameters<typeof detectColorDepth>["length"] extends 0 ? true : false>,
            // Result should be assignable to ColorDepth
            Expect<typeof result extends ColorDepth ? true : false>,
        ];
